...
function normalizeSignalMessage(message, studySessionId) {
  if (!SIGNAL_EVENT_TYPES.includes(message?.type)) {
    return null;
  }

  const payload = message?.payload || {};
  const scopedSessionId = toNumber(payload.studySessionId);

  if (scopedSessionId !== studySessionId) {
    return null;
  }

  if (payload.scope && payload.scope !== 'study_session') {
    return null;
  }
...
export function useStudySessionWebRTC({
  enabled,
  studySessionId,
  userId,
  participants,
  ...
  const [statusMessage, setStatusMessage] = useState('로컬 미디어가 준비되면 스터디 영상 연결을 시작할 수 있습니다.');
  ...
  async function flushPendingCandidates(targetUserId, peer) {
    ...
    for (const candidate of candidates) {
      try {
        ...
        console.error('Failed to apply pending study ICE candidate:', error);
      }
    }
  }
  ...
  function getOverallStatus() {
    ...
    if (states.some((state) => state === 'connected')) {
      return {
        status: 'connected',
        message: '하나 이상의 참가자와 영상 연결이 수립되었습니다.',
      };
    }
    ...
    if (states.some((state) => state === 'connecting' || state === 'signaling')) {
      return {
        status: 'connecting',
        message: '스터디 참가자와 영상 연결을 협상 중입니다...',
      };
    }
  ...
  function ensurePeerConnection(targetUserId, streamOverride) {
    if (typeof RTCPeerConnection === 'undefined') {
      ...
      setStatus('error', '현재 환경에서는 스터디 영상 연결을 사용할 수 없습니다.');
      return null;
    }
    ...
    peer.onconnectionstatechange = () => {
      ...
      if (state === 'failed') {
        setErrorMessage('스터디 영상 연결에 실패한 참가자가 있습니다.');
        setStatus('error', '일부 스터디 참가자와 연결하지 못했습니다. 다시 시도해 주세요.', {
          canRetry: true,
        });
        return;
      }
    ...
    peer.oniceconnectionstatechange = () => {
      if (peer.iceConnectionState === 'failed') {
        setErrorMessage('스터디 ICE 연결에 실패한 참가자가 있습니다.');
        setStatus('error', '스터디 ICE 연결에 실패했습니다. 네트워크를 확인한 뒤 다시 시도해 주세요.', {
          canRetry: true,
        });
      }
    };
  ...
  async function createOffer(targetUserId, streamOverride) {
    ...
    try {
      ...
      setStatus('error', '스터디 연결 제안을 만들지 못했습니다.', { canRetry: true });
      return false;
    }
  }
  ...
  async function createAnswer(senderUserId, signal) {
    if (!localStreamRef.current) {
      setErrorMessage('스터디 응답 전에 로컬 미디어를 먼저 준비해 주세요.');
    ...
    try {
      ...
      setErrorMessage(getConnectionErrorMessage(error));
      setStatus('error', '받은 스터디 연결 제안을 처리하지 못했습니다.', { canRetry: true });
    }
  }
  ...
  async function applyAnswer(senderUserId, signal) {
    ...
    if (!peer) {
      setStatus('disconnected', '스터디 연결 준비 전에 응답이 도착했습니다. 다시 시도해 주세요.', {
        canRetry: true,
      });
      return;
    }
    ...
    try {
      ...
      setErrorMessage(getConnectionErrorMessage(error));
      setStatus('error', '스터디 연결 응답을 적용하지 못했습니다.', { canRetry: true });
    }
  }
  ...
  async function applyIceCandidate(senderUserId, signal) {
    ...
    try {
      ...
      console.error('Failed to apply study ICE candidate:', error);
    }
  }
    ...
    try {
      ...
      if (controlMessage) {
        ...
        if (controlMessage.type === 'ws_error') {
          ...
          setStatus('error', controlMessage.message || '스터디 영상 연결 신호 처리에 실패했습니다.', {
            canRetry: true,
          });
          return;
        }
      ...
      const signal = normalizeSignalMessage(parsedMessage, studySessionId);
      ...
      setErrorMessage('스터디 시그널링 메시지를 해석하지 못했습니다.');
      setStatus('error', '스터디 시그널링 메시지를 해석하지 못했습니다.', { canRetry: true });
    }
  }
  ...
  async function openSignalingSocket() {
    ...
    try {
      ...
      nextSocket.onclose = () => {
        ...
        if (startRequestedRef.current) {
          setStatus('disconnected', '스터디 시그널링 소켓이 종료되었습니다. 다시 시도해 주세요.', {
            canRetry: true,
          });
        } else {
          setStatus('not_connected', '스터디 시그널링 소켓이 종료되었습니다.');
        }
      };

      return nextSocket;
    } catch (error) {
      setErrorMessage('스터디 시그널링 소켓을 열지 못했습니다.');
      setStatus('error', '스터디 시그널링 소켓을 열지 못했습니다.', { canRetry: true });
      return null;
    }
  }
  ...
  async function startConnection(streamOverride) {
    ...
    if (!activeStream) {
      setErrorMessage('스터디 영상 연결 전에 로컬 미디어가 필요합니다.');
      setStatus('error', '스터디 영상 연결 전에 로컬 미디어를 준비해 주세요.');
      return false;
    }

    if (participantUserIds.length === 0) {
      setStatus('preparing', '아직 연결할 다른 스터디 참가자가 없습니다.');
      return false;
    }
    ...
    if (!isSocketReadyRef.current) {
      setStatus('preparing', '스터디 시그널링 소켓을 여는 중입니다...');
      return true;
    }
  ...
  async function retryConnection(streamOverride) {
    ...
    setErrorMessage('');
    closeAllPeers();
    setStatus('not_connected', '스터디 영상 연결을 다시 준비하는 중입니다...');
  ...
  function stopConnection() {
    ...
    closeAllPeers();
    setStatus('not_connected', '스터디 영상 연결을 중지했습니다.');
  }

  useEffect(() => {
    if (!enabled || !studySessionId || !userId) {
      closeSignalingSocket({ resetRequestedStart: true });
      closeAllPeers();
      setErrorMessage('');
      setLastSignalType('');
      setStatus('not_connected', '스터디 영상 연결 대기 중입니다.');
      return undefined;
    }
