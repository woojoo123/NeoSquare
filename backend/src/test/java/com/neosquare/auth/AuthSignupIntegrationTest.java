...
class AuthSignupIntegrationTest {
    ...
    @Test
    void signupAcceptsSimplePassword() throws Exception {
        mockMvc.perform(post("/api/auth/signup")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {
                ...
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.email").value("simple@example.com"))
                .andExpect(jsonPath("$.data.nickname").value("simple-user"));

        User savedUser = userRepository.findByEmail("simple@example.com").orElseThrow();
